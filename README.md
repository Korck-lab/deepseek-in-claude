...
- **Provider models in the picker** — models are fetched live from each provider's API (10-minute cache) and merged into Anthropic's model list, so `deepseek-v4-flash`, `deepseek-v4-pro`, and `claude-mimo-v2.5` (when `XIOMIMIMO_API_KEY` is set) show up natively and you select them with `/model` like any other model.
...
- **Paste a screenshot and it still works** — the base DeepSeek V4 models (`deepseek-v4-flash`, `deepseek-v4-pro`) can't see images, and they don't say so — send one a screenshot and it answers anyway, describing something that isn't there. (`deepseek-v4-flash-vision-exp` can see and keeps its own image turns.) The proxy spots the image before dispatch and sends that one turn to a model that can see it, then answers as the model you picked, so the next turn is back on DeepSeek and your 1M window is intact. By default that's `claude-sonnet-5` at medium effort on the Claude plan you're already logged into — nothing to install, but it does spend plan traffic per image. Set `vision.redirect: true` and image turns go to a local vision model instead (LM Studio by default, `prism-ml/bonsai-27b`), which costs nothing but your own GPU; that one stays off until you ask, because it needs a server you're running and sends your prompt there. `vision.anthropic: false` turns the plan tier off and gives you the blind answer back, with a warning. See ADR-0004.
- **Vision-capable models are left alone** — the redirect fires on capability, not on brand. A capability reported by the provider's model list wins, an id that names itself (`deepseek-v4-flash-vision-exp`, `qwen2-vl-7b`) is read as able to see, and `capabilities:` in `config.yml` overrides both. So picking DeepSeek's own vision model keeps your image turns on DeepSeek instead of quietly spending plan traffic to answer what it could already see.
...
- **Second provider: Xiaomi MiMo** — set `XIOMIMIMO_API_KEY` in `.env` and `mimo-v2.5` / `mimo-v2.5-pro` join the picker as `claude-mimo-v2.5` / `claude-mimo-v2.5-pro[1m]`. The same leg machinery — display ids, model-list merge, routing, usage log — serves any number of Anthropic-compatible providers. DeepSeek keeps its known 1M window claim; Xiaomi only claims `[1m]` when its model list reports a window at or over 750k. Audio-only `-asr`/`-tts` models are filtered out of the picker. MiMo can see, so it is pinned vision-capable in `config.yml` and its image turns stay on Xiaomi. See ADR-0007.
- **Usage observability** — every provider request appends one JSON line with the real input/cache/output token counts to `logs/proxy-usage.jsonl`. Message content and auth headers are never logged.
...
1. Intercepts `/v1/models` and merges the live Anthropic model list with the live provider lists — DeepSeek always, Xiaomi MiMo when `XIOMIMIMO_API_KEY` is set.
2. For every request whose `model` is a provider id, rewrites `authorization` → `x-api-key` and forwards to that provider's Anthropic-compatible base (`$DEEPSEEK_ANTHROPIC_BASE_URL`, default `https://api.deepseek.com/anthropic`; `$XIOMIMIMO_ANTHROPIC_BASE_URL`).
...
The `vision` block has two tiers, tried in order. `redirect: true` sends image turns to a local vision model — `baseUrl` is your OpenAI-compatible local server (LM Studio by default) and `model` the vision-capable id it reports. It is off until you set it, because it needs a server you are actually running and sends your prompt and image to it, so it is not something to start doing on its own. Behind it, `anthropic:` sends image turns to the Anthropic leg on the credential the bridge already holds — `claude-sonnet-5` at `medium` effort unless you say otherwise, and both keys take any Anthropic model id and any of `low`/`medium`/`high`/`xhigh`/`max` (or `effort: false` to send no effort field at all, which older models like Haiku 4.5 require — they reject the parameter). That tier is on by default: it needs nothing running, and the alternative for an image turn is a confidently wrong answer. It does spend plan traffic per image, so `anthropic: false` turns it off — with both tiers off, image requests go to the vision-less model blind (unless the picked model is itself vision-capable) and the proxy prints which settings would have handled them. `capabilities` is the final say on what a model can do, for the cases the two rules below it get wrong. Vision is resolved in precedence order: an explicit `capabilities:` entry, then whatever the provider's model list reported (`capabilities.image_input.supported` — Anthropic reports it, DeepSeek does not), then a default that reads the id itself — the `claude-*` family can see, and so can any id naming `vision` or `vl`, which is how `deepseek-v4-flash-vision-exp` keeps its own image turns. Anything else is assumed blind.

## Configuration

All variables come from `.env` or real environment variables (real vars win). Only the DeepSeek key is required; the Xiaomi MiMo key is optional and enables the second provider.
...
| `XIOMIMIMO_API_KEY` | — | Optional. Xiaomi MiMo API key; enables the second provider. |
| `XIOMIMIMO_BASE_URL` | `https://token-plan-sgp.xiaomimimo.com/anthropic` | Xiaomi MiMo base URL (may carry the `/anthropic` suffix; stripped for model discovery). |
| `XIOMIMIMO_ANTHROPIC_BASE_URL` | `$XIOMIMIMO_BASE_URL` | Xiaomi's Anthropic-compatible endpoint. |
