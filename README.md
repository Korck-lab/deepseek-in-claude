...
- **Paste a screenshot and it still works — if you opt in** — DeepSeek V4 can't see images, so an image in your prompt comes back a hard 400. Set `vision.redirect: true` and the proxy spots the image before dispatch and sends that one turn to a local vision model instead — LM Studio by default (`prism-ml/bonsai-27b`), so no Anthropic plan traffic is spent on images — then answers as the model you picked, so the next turn is back on DeepSeek and your 1M window is intact. It stays off until you ask, because it needs a server you're running and sends your prompt there. Point `vision.baseUrl` at that server and `vision.model` at a vision-capable id, or tell the proxy which models can see with `capabilities:`. See ADR-0004.
...
The `vision` block is opt-in, and off until you set `redirect: true` — the redirect needs a local server you are actually running, and it sends your prompt and image to it, so it is not something to start doing on its own. Turned on, it routes image turns to a local vision model: `baseUrl` is your OpenAI-compatible local server (LM Studio by default) and `model` the vision-capable id it reports. Left off, image requests fail upstream and the proxy prints which setting would have handled them. `capabilities` is the escape hatch for models whose provider doesn't advertise what they can do: anything you mark `vision: true` there is never redirected, and Anthropic's own models are read straight from its model list.
