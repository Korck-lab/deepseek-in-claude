...
- **Paste a screenshot and it still works** — DeepSeek V4 can't see images, so an image in your prompt would come back a hard 400. The proxy spots the image before dispatch and sends that one turn to a model that can see it, then answers as the model you picked, so the next turn is back on DeepSeek and your 1M window is intact. By default that's `claude-sonnet-5` at medium effort on the Claude plan you're already logged into — nothing to install, but it does spend plan traffic per image. Set `vision.redirect: true` and image turns go to a local vision model instead (LM Studio by default, `prism-ml/bonsai-27b`), which costs nothing but your own GPU; that one stays off until you ask, because it needs a server you're running and sends your prompt there. `vision.anthropic: false` turns the plan tier off and gives you the 400 back. See ADR-0004.
